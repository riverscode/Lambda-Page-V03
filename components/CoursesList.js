...
const CoursesList = ({ courses }) => {
  ...
  const filteredBlogCourses = courses
    .sort(
      (a, b) =>
    ...
    <div className="mt-16 md:mt-32 px-6 md:px-0 flex flex-col max-w-7xl mx-auto text-white ">
      <div className="flex items-end">
        ...
        <span className="font-light text-md md:text-xl ml-1 md:ml-3">
          | {filteredBlogCourses.length} cursos disponibles
        </span>
      </div>
      ...
      <div className="grid grid-cols-1 md:grid-cols-3 gap-6 mt-16 ">
        {!filteredBlogCourses.length && "No hemos encontrado ningún Curso 🥺"}
        {filteredBlogCourses.map((course, index) => {
          return (
            <CourseCard
              key={index}
              title={course.title}
